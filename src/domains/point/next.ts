...
import { ActionType, RedisAction, RedisClientType } from 'types/action'
...
const gameModel: IGameModel = container.resolve('gameModel')
const pointModel: IPointModel = container.resolve('pointModel')
const actionModel: IActionModel = container.resolve('actionModel')
const redisClient: RedisClientType = container.resolve('redisClient')

export const handleCurrentPointUpdates = async (gameId: string, team: TeamNumber, pointId: string): Promise<IPoint> => {
...
const handlePointScore = async (game: IGame, team: TeamNumber, point: Document<unknown, unknown, IPoint> & IPoint) => {
    const totalActions = await redisClient.get(`${game._id.toHexString()}:${point._id.toHexString()}:${team}:actions`)
